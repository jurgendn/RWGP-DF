...
from consts.bitcoin_otc import BatchSizeConfig, BoundedValue, GlobalValues
...
def make_dataset(
    ...
    G, temporal_changes = load_txt_dataset(
        file_path="dataset/soc-sign-bitcoinotc.csv",
        source_idx=0,
        target_idx=1,
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        if should_log_to_mlflow(target, gp_df_res, static_louvain_res):
            with mlflow.start_run():
                ...
                mlflow.log_artifact("dataset/soc-sign-bitcoinotc.csv", "dataset.txt")
...
def runner():
    for params in BatchSizeConfig().get_list():
        print("Running for soc-sign-bitcoin-otc graph")

...
def split_community(
    graph: nx.Graph, community: List[int], steps: int = 5
...
def separate_communities_v3(
    ...
    def delta_modularity_keep(
        ...
        sum_kj = sum(degrees[j] for j in community_set)
        delta_q = (l_iC / total_edges) - (d_node * sum_kj) / (
            4 * total_edges * total_edges
        )
        return delta_q
    ...
    def adjust_communities(
        ...
        for node in list(C1):  # Convert to list to avoid modification during iteration
            delta_keep = delta_modularity_keep(
                graph, list(C1), node, degrees, total_edges
            )
            delta_add = delta_modularity_add(
                graph, list(C2), node, degrees, total_edges
            )
    ...
    def refine_partition(
        graph: nx.Graph, community: List[int], degrees: Dict[int, int], total_edges: int
    ) -> Tuple[List[int], List[int]]:
        """
        ...
        while (adjustments != 0 or iteration < 2) and iteration < max_iterations:
            adjustments, C1, C2 = adjust_communities(
                graph, C1, C2, degrees, total_edges
            )
            iteration += 1
        return C1, C2
    ...
    def validate_division(
        ...
        try:
            ...
            new_mod = compute_modularity(graph, community1) + compute_modularity(
                graph, community2
            )
...
def separate_communities_v3_ultra_fast(
    ...
    viable_communities = {
        k: v for k, v in community_groups.items() if 3 <= len(v) <= 50
    ...
    for comm_id, nodes in viable_communities.items():
        ...
        for node in high_degree[:sample_size]:
            cross_edges += sum(
                1 for neighbor in adj_list[node] if neighbor in low_degree
            )

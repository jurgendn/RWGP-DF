...
def separate_communities_v4(
    ...
    def simple_split(community_nodes: List[int]) -> Tuple[List[int], List[int]]:
        ...
        while queue and len(group1) < target_size:
            ...
            neighbors = [
                n for n in adj[node] if n in community_nodes and n not in visited
            ]
        ...
        return (
            group1,
            group2 if group2 else community_nodes[: len(community_nodes) // 2],
            community_nodes[len(community_nodes) // 2 :],
        )

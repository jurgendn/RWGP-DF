...
def degree_map(
    G: nx.Graph, nodes: Iterable[int], weight: Optional[str] = "weight"
) -> Dict[int, float]:
...
def cut_weight_between(
    G: nx.Graph, S1: Set[int], S2: Set[int], weight: Optional[str] = "weight"
) -> float:
...
def rw_bipartition_sparse(
    G: nx.Graph, community: Iterable[int], steps: int = 10
    ...
    Dinv = csr_matrix(
        (1.0 / safe_deg, (np.arange(len(nodes)), np.arange(len(nodes)))), shape=A.shape
    )
    P = Dinv @ A
    ...
    thresh = (
        deg_arr / deg_arr.sum()
        if deg_arr.sum() > 0
        else np.full_like(deg_arr, 1.0 / len(nodes))
    )
...
class RWGPRefinement:
    ...
    def _update_partition_with_split(
        self, part: Dict[int, int], cid: int, S1: Iterable[int], S2: Iterable[int]
    ) -> Dict[int, int]:
    ...
    def forward(
        self, G: nx.Graph, original: Dict[int, int], rounds: int = 1
    ) -> Dict[int, int]:
        part = original.copy()
...
def separate_communities_v5(
    ...
    method = RWGPRefinement(
        num_walks=10, resolution=1.0, weight="weight", min_size=10, eps=1e-9
    )

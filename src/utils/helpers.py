...
def capture_graph_statistics(
    ...
    for community_id in range(num_communities):
        community_nodes = [
            node
            for node, cid in initial_communities_dict.items()
            if cid == community_id
        ]
        if community_nodes:
    ...
    if communities_list:
        stats["initial_modularity"] = nx.algorithms.community.modularity(
            graph, communities_list
        )
    else:
    ...
    if nx.is_connected(graph):
        ...
        stats["average_shortest_path_length"] = nx.average_shortest_path_length(
            largest_subgraph
        )
    ...
    if temporal_changes:
        ...
        stats["deletion_intensity"] = (
            total_deletions / stats["num_edges"] if stats["num_edges"] > 0 else 0
        )
        stats["insertion_intensity"] = (
            total_insertions / stats["num_edges"] if stats["num_edges"] > 0 else 0
        )
        ...
        stats["max_deletions_per_step"] = (
            max(deletions_per_step) if deletions_per_step else 0
        )
        stats["std_insertions_per_step"] = np.std(insertions_per_step)
        stats["max_insertions_per_step"] = (
            max(insertions_per_step) if insertions_per_step else 0
        )
...
def generate_plot_filename(mode: str, dataset_config: Dict[str, Any]) -> str:
    ...
    common_suffix = (
        f"initial_fraction_{initial_fraction}_{load_type}_benchmark_plot.png"
    )
